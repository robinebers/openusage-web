...
- Removed the sticky "OpenUsage has changed" banner (`components/announcement-banner.tsx` trashed) and the muted "Looking for v0.6.28?" link from both CTAs. `LEGACY_VERSION` still drives contributor backfill only.
